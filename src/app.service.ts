...
export class AppService {
  ...
  // 1. Método que consulta la PokéAPI externa
  async buscarPokemon(nombreOrId: string) {
    if (!nombreOrId) return null;

    try {
      const response = await fetch(
        `https://pokeapi.co/api/v2/pokemon/${nombreOrId.toLowerCase().trim()}`,
      );
      ...
      return {
        ...
        tipos: data.types.map((t: any) => t.type.name).join(', '),
      };
    } catch (error) {
